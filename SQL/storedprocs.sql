...
BEGIN
   DECLARE meetupId INT;
	START TRANSACTION;
        ...
        SET meetupId = LAST_INSERT_ID();
        
        CALL add_meetup_interests(meetupId, _interets);
    COMMIT;
    SELECT meetupId;
...
BEGIN
    ...
    DELETE FROM meetups_interests WHERE id_meetup = meetupId ;
    ...
    WHILE LENGTH(interetList) > 0 DO
        SET interetId = SUBSTRING_INDEX(interetList, ',', 1);
        IF interetId <> '' THEN
            INSERT INTO meetups_interests (id_meetup, id_interest) VALUES (meetupId, interetId);
        END IF;
...
DROP PROCEDURE IF EXISTS DeleteMeetup;
DELIMITER //
CREATE PROCEDURE DeleteMeetup(IN meetupId INT)
BEGIN
    START TRANSACTION;

    DELETE FROM meetups_users
    WHERE id_meetup = meetupId;

    DELETE FROM meetups_interests
    WHERE id_meetup = meetupId;
